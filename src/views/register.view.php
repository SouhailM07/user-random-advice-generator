...
<body>
    <main class="min-h-screen flexCenter bg-[var(--blue-900)]">
        <form  action="/register" method="post" class="flex flex-col text-white w-[28rem] space-y-[1rem]">
                        <h1 class="text-white mb-[1rem]! text-center text-[2rem] textGreen font-bold">Register</h1>
            <?php
            ...
            <div class="space-y-4 flex flex-col">    
                <label for="confirmPassword" class=" text-[1.2rem]">Confirm Password</label>
                <input required placeholder="..." id="confirmPassword" name="confirmPassword" type="text" class="p-2 rounded-md outline-none border-2 border-white">
            </div>
            <button class="bg-[var(--primary-green)] p-2 rounded-md text-[1.2rem] text-black hover:scale-110 duration-200">Submit</button>
                    <div>
                 <p class="text-white mt-4 text-center">You have account? <a href="/login" class="text-[var(--primary-green)] hover:underline"> login</a></p>
                </div>
        </form>
    </main>
</body>
